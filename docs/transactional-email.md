...
YuhBusiness uses the dedicated sender subdomain `mail.yuhbusiness.com`. Publish
the SPF, MX, and DKIM records generated by Resend, add DMARC, and use
`YuhBusiness <notifications@mail.yuhbusiness.com>`. Configure that same
verified sender in Supabase Auth SMTP for confirmation, invitation, and
password-reset email. These mail records do not replace the website's apex A
record.
...
3. Add `mail.yuhbusiness.com` under Resend > Domains.
4. In Spaceship DNS, copy the SPF TXT, SPF MX, and DKIM TXT records exactly as
   Resend displays them. Spaceship normally appends `.yuhbusiness.com`, so the
   host labels will typically end in `.mail` rather than repeating the full
   domain. Confirm the final public names in Resend before saving.
...
select vault.create_secret(
  'https://yuhbusiness.com/api/email/process',
...
```text
POST https://yuhbusiness.com/api/email/process
...
Sender email: notifications@mail.yuhbusiness.com
```

Keep email confirmation enabled. Do not save the production sender until
...
Verify `mail.yuhbusiness.com` in Resend, publish SPF/DKIM and DMARC, update both
`RESEND_FROM_EMAIL` and the Supabase SMTP sender to
`notifications@mail.yuhbusiness.com`, and test Gmail/Outlook/mobile rendering
