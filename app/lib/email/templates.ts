...
export function buildTransactionalEmail(
  ...
  if (input.eventType === "WELCOME_ACCOUNT") {
    ...
    content =
      paragraph(
        `${stringValue(payload, "inviter_name", "The business owner")} invited you as ${stringValue(payload, "role", "staff")}. Sign in with this email address to accept the invitation.`,
    ...
    subject =
      input.subject?.trim() ||
      `${ORDER_HEADLINES[input.eventType]} · ${orderNumber}`;
    title = ORDER_HEADLINES[input.eventType] || subject;
    preview = `${orderNumber} has a new status.`;
    content =
      paragraph(`Hi ${name}, here is the latest update from ${business}.`) +
      orderItems(payload) +
      detailRows([
        ["Order", orderNumber],
    ...
    subject =
      input.subject?.trim() || `Your appointment with ${business} is confirmed`;
    title = "Appointment confirmed";
    preview = `Your appointment with ${business} is confirmed.`;
    content =
      paragraph(`Hi ${name}, ${business} confirmed your appointment.`) +
      detailRows([
        ["Service", stringValue(payload, "service_name", "Appointment")],
        [
          "When",
          dateTime(
            stringValue(payload, "starts_at"),
            stringValue(payload, "timezone", "America/Belize"),
          ),
        ],
    ...
    content =
      paragraph(
        `Hi ${name}, your appointment with ${business} has been cancelled.`,
      ) +
      detailRows([
        ["Service", stringValue(payload, "service_name", "Appointment")],
        [
    ...
    subject =
      input.subject?.trim() || `Reminder: your appointment with ${business}`;
    title = "Appointment reminder";
    preview = `Your appointment with ${business} is coming up.`;
    content =
      paragraph(
        `Hi ${name}, this is a reminder about your upcoming appointment.`,
      ) +
      detailRows([
        ["Business", business],
        ["Service", stringValue(payload, "service_name", "Appointment")],
    ...
    content =
      detailRows([
        ["From", name],
        ["Email", stringValue(payload, "sender_email")],
    ...
    subject =
      input.subject?.trim() ||
      (expired
        ? `${business}'s trial has ended`
        : `${business}'s trial is ending soon`);
    ...
    title =
      input.eventType === "SUBSCRIPTION_PAYMENT_ISSUE"
    ...
    content =
      paragraph(
        input.eventType === "SUBSCRIPTION_PAYMENT_ISSUE"
          ...
          : `The subscription for ${business} has been updated successfully.`,
      ) +
      detailRows([
        ["Business", business],
        ["Plan", plan],
    ...
    subject =
      input.subject?.trim() ||
      stringValue(payload, "title", "Important business alert");
    ...
    content =
      paragraph(
        stringValue(
          payload,
          "message",
          "There is important activity requiring your attention.",
        ),
      ) +
      action(
        "Open YuhBusiness",
