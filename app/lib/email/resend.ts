...
import { buildTransactionalEmail, type TransactionalEmailInput } from "./templates";
...
function client() {
  if (!resendClient) resendClient = new Resend(requiredServerEnv("RESEND_API_KEY"));
  return resendClient;
}

export async function sendTransactionalEmail(input: SendTransactionalEmailInput) {
  const recipient = input.to.trim().toLowerCase();
  if (!isValidEmailAddress(recipient)) throw new Error("The recipient email address is invalid.");

  const replyTo = input.replyTo?.trim().toLowerCase();
  if (replyTo && !isValidEmailAddress(replyTo)) throw new Error("The reply-to email address is invalid.");
  ...
  const { data, error } = await client().emails.send(
    {
      ...
      tags: [{ name: "event", value: input.eventType.toLowerCase().replaceAll("_", "-").slice(0, 256) }],
    },
