...
export async function PATCH(request: Request, context: RouteContext) {
  try {
    ...
    if (!VALID_PLANS.has(plan) || !VALID_STATUSES.has(status)) {
      return Response.json({ error: "Choose a valid plan and access status." }, { status: 400 });
    }
    if (status === "trial" && (
      trialDays !== null && (!Number.isInteger(trialDays) || trialDays < 1 || trialDays > 365)
    )) {
      return Response.json({ error: "Trial days must be a whole number between 1 and 365." }, { status: 400 });
    }
    ...
    if (!profile?.is_active || profile.platform_role?.toUpperCase() !== "SUPER_ADMIN") {
      return Response.json({ error: "Only a platform super admin can change subscription access." }, { status: 403 });
    }
    ...
    if (status === "trial") {
      ...
      updates.trial_ends_at = new Date(Date.now() + days * 86_400_000).toISOString();
    }
    ...
    const { data: tenant, error: updateError } = await admin
      ...
      .maybeSingle();
    if (updateError) {
      const message = updateError.message || "Unable to save subscription access.";
      const statusCode = message.includes("staff") || message.includes("seat") ? 409 : 400;
    ...
    const { error: auditError } = await admin.from("team_access_events").insert({
      tenant_id: tenantId,
    ...
    const message = error instanceof Error ? error.message : "Unable to save subscription access.";
    console.error("[subscription-admin]", message);
