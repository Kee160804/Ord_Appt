...
//           <p className="text-slate-500">Super admin overview — all tenants</p>
//         </div>
//         <button
//           onClick={logout}
//           className="px-4 py-2 text-sm font-medium text-slate-600 hover:text-slate-900 bg-slate-100 rounded-xl hover:bg-slate-200 transition"
//         >
//           Logout
//         </button>
//       </div>
...
//         <StatCard label="Total Revenue" value={formatCurrency(totalRevenue)} icon={<DollarSign className="w-5 h-5" />} bg="bg-emerald-50" />
//         <StatCard label="Total Transactions" value={totalActivity.toString()} icon={<ShoppingBag className="w-5 h-5" />} bg="bg-blue-50" />
//         <StatCard label="Total Customers" value={totalCustomers.toString()} icon={<Users className="w-5 h-5" />} bg="bg-violet-50" />
//         <StatCard label="Active Tenants" value={mockTenants.length.toString()} icon={<Building2 className="w-5 h-5" />} bg="bg-amber-50" />
//       </div>

//       {/* Tenant list */}
//       <Card>
//         <CardHeader>
//           <h3 className="font-semibold text-slate-900">All Businesses</h3>
//         </CardHeader>
//         <div className="divide-y divide-slate-50">
//           {mockTenants.map(tenant => {
//             const analytics = mockAnalytics[tenant.id];
//             return (
//               <div key={tenant.id} className="px-6 py-4 flex items-center gap-4 hover:bg-slate-50 transition-colors">
//                 <div className="w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold" style={{ backgroundColor: tenant.logoBg }}>
//                   {tenant.logo}
//                 </div>
//                 <div className="flex-1 min-w-0">
//                   <div className="flex items-center gap-2">
//                     <p className="text-sm font-semibold text-slate-900">{tenant.name}</p>
//                     <span className={`text-xs px-2 py-0.5 rounded-full ${tenant.businessType === 'appointment' ? 'bg-violet-100 text-violet-700' : 'bg-orange-100 text-orange-700'}`}>
//                       {tenant.businessType}
//                     </span>
//                   </div>
