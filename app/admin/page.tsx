...
function RoleView({ roles, isLoading }: { roles: AdminRoleSummary[]; isLoading: boolean }) {
  return <ManagementPanel description="Read-only platform audit. Each business owner manages Manager and Staff access from their own Settings > Team & Access." title="Access Oversight"><div className="grid gap-4 p-5 md:grid-cols-2 2xl:grid-cols-3">{isLoading && [1, 2, 3].map((item) => <div key={item} className="h-56 animate-pulse rounded-2xl bg-slate-800/60 light:bg-slate-100" />)}{!isLoading && roles.length === 0 && <div className="md:col-span-2"><EmptyState message="No role records were returned by Supabase." /></div>}{roles.map((role, index) => <RoleCard key={role.id} index={index} role={role} />)}</div></ManagementPanel>;
}

function RoleCard({ role, index }: { role: AdminRoleSummary; index: number }) {
  const permissionMap: Record<string, string[]> = { "SUPER ADMIN": ["View platform dashboards", "Manage subscriptions", "Approve paid seats", "Audit tenant access", "View analytics", "System settings"], OWNER: ["View dashboard", "Manage team access", "Edit storefront", "Manage appointments", "Manage orders", "Manage customers"], ADMIN: ["Legacy business role", "Manage business data", "Manage daily activity", "View analytics"], MANAGER: ["View dashboard", "Manage business data", "Manage daily activity", "View customers"], STAFF: ["View dashboard", "Manage daily orders or appointments"] };
...
function PlanBadge({ plan }: { plan: Tenant["plan"] }) { return <span className="rounded-full bg-violet-500/10 px-2.5 py-1 text-[10px] font-semibold text-violet-400 light:text-violet-700">{PLAN_DEFINITIONS[plan].name}</span>; }
