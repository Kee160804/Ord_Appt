...
type DemoSection =
  "overview" | "activity" | "catalog" | "customers" | "analytics" | "settings";
...
const panelClass =
  "rounded-2xl border border-slate-700 bg-slate-900 light:border-slate-200 light:bg-white";
...
export function DemoDashboardPreview({ tenant }: { tenant: Tenant }) {
  ...
  const navItems: {
    ...
    {
      id: "activity",
      label: isAppointment ? "Appointments" : "Orders",
      icon: isAppointment ? CalendarDays : ShoppingBag,
    },
    {
      id: "catalog",
      label: isAppointment ? "Services" : "Products",
      icon: isAppointment ? Scissors : Package,
    },
  ...
  const addDemoItem = () => {
    ...
    setCatalogItems((current) => [
      {
        id: `demo-${Date.now()}`,
        name: isAppointment
          ? `New Demo Service ${suffix}`
          : `New Demo Product ${suffix}`,
    ...
    showNotice(
      `Demo ${isAppointment ? "service" : "product"} added. It will reset when you refresh.`,
    );
  };
    ...
    <div className="flex min-h-[calc(100vh-112px)] bg-[#08111f] text-white light:bg-[#f6f8fc] light:text-slate-900">
      <aside className="hidden w-56 shrink-0 flex-col border-r border-white/5 bg-[#111a35] md:flex">
        <div className="flex items-center gap-3 border-b border-white/5 px-5 py-5">
          ...
          <div>
            <p className="text-sm font-bold">YuhBusiness</p>
        ...
        <div className="border-b border-white/5 p-4">
          <div className="flex items-center gap-3 rounded-xl bg-white/5 p-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[9px] capitalize text-violet-300">
        ...
        <nav className="space-y-1 p-3" aria-label="Demo dashboard navigation">
          {navItems.map(({ id, label, icon: Icon }) => (
            <button
              ...
              className={cn(
                "flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-xs transition",
                activeSection === id
        ...
        <div className="mt-auto space-y-3 border-t border-white/5 p-4">
          <p className="text-[10px] leading-4 text-slate-500">
            Everything here is temporary sample data.
          </p>
          <Link
            href="/register"
            className="flex items-center justify-center gap-2 rounded-lg bg-violet-600 px-3 py-2.5 text-xs font-bold text-white transition hover:bg-violet-500"
          >
            Create your business <ArrowRight className="h-3.5 w-3.5" />
          </Link>
        </div>
      </aside>
        ...
        <div className="mx-auto max-w-6xl space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-400 light:text-slate-600">
                Explore how you would manage {settingsForm.businessName}
              </p>
            </div>
            <div className="flex items-center gap-2">
              <span className="rounded-full border border-violet-500/30 bg-violet-500/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-violet-300 light:text-violet-700">
                Interactive demo
              </span>
              <Link
                href="/register"
                className="rounded-xl bg-violet-600 px-4 py-2.5 text-xs font-bold text-white hover:bg-violet-500"
              >
                Start free
              </Link>
            </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  activeSection === id
                    ...
                    : "bg-slate-900 text-slate-400 light:bg-white light:text-slate-600",
                )}
              >
          ...
          {activeSection === "settings" && (
            <SettingsSection
              ...
              onSave={() =>
                showNotice(
                  "Preview settings saved locally. Create an account to publish them.",
                )
              }
            />
          )}
          ...
          <section className="flex flex-col items-center justify-between gap-4 rounded-2xl border border-violet-500/25 bg-gradient-to-r from-violet-950/70 to-indigo-950/50 p-5 text-center sm:flex-row sm:text-left light:from-violet-50 light:to-indigo-50">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-400 light:text-slate-600">
                Create your business, customize your storefront, and start
                accepting customers.
              </p>
            </div>
            <Link
              href="/register"
              className="inline-flex shrink-0 items-center gap-2 rounded-xl bg-violet-600 px-5 py-3 text-xs font-bold text-white hover:bg-violet-500"
            >
      ...
      {notice && (
        <div
          role="status"
          className="fixed bottom-3 left-3 right-3 z-50 flex max-w-sm items-start gap-3 rounded-xl border border-emerald-500/30 bg-slate-950 px-4 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] text-xs text-emerald-300 shadow-2xl light:bg-white light:text-emerald-700 sm:bottom-5 sm:left-auto sm:right-5"
        >
    ...
    <>
      <section className="relative min-h-36 overflow-hidden rounded-2xl border border-slate-700 bg-slate-900 light:border-slate-200 light:bg-white">
        ...
        <div className="relative z-10 p-6">
          ...
          <p className="mt-1 text-xs text-slate-400 light:text-slate-600">
            Your daily performance and customer activity appears here.
          </p>
        </div>
      </section>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-[10px] text-slate-500">
                Sample seven-day performance
              </p>
            </div>
            <TrendingUp className="h-4 w-4 text-violet-400" />
          </div>
          <div className="mt-5 flex h-40 items-end gap-2 border-b border-slate-700 px-2 light:border-slate-200">
        ...
        <div className={cn(panelClass, "p-5")}>
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold">
              {isAppointment ? "Top Services" : "Top Products"}
            </h3>
            <button
            ...
            {catalogItems
              ...
              .map((item, index) => (
                <div key={item.id} className="flex items-center gap-3">
                  <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-violet-500/15 text-xs font-bold text-violet-300">
                    {index + 1}
                  </span>
      ...
      <section className={panelClass}>
        <div className="flex items-center justify-between border-b border-slate-700 px-5 py-4 light:border-slate-200">
          <h3 className="text-sm font-bold">
            Recent {isAppointment ? "Appointments" : "Orders"}
          </h3>
          <button
        ...
        <div className="divide-y divide-slate-800 light:divide-slate-100">
...
function ActivitySection({
  ...
  filter: string;
  isAppointment: boolean;
  onAdvance: (activity: DemoActivity) => void;
  ...
  const filtered = activities.filter(
    (activity) =>
      ...
      [activity.customerName, activity.reference].some((value) =>
  ...
  return (
    <section className={cn(panelClass, "overflow-hidden")}>
      <div className="flex flex-wrap gap-3 border-b border-slate-700 p-4 light:border-slate-200">
        <SearchBox
          value={search}
          onChange={onSearch}
          placeholder={`Search ${isAppointment ? "appointments" : "orders"}...`}
        />
        <select
          value={filter}
          onChange={(event) => onFilter(event.target.value)}
          className="rounded-xl border border-slate-700 bg-slate-800 px-3 py-2 text-xs outline-none light:border-slate-200 light:bg-white"
        >
      ...
      <div className="divide-y divide-slate-800 light:divide-slate-100">
        {filtered.map((activity) => (
          ...
          >
            ...
            />
            <button
              onClick={() => onAdvance(activity)}
              className="ml-auto rounded-lg border border-violet-500/30 px-3 py-2 text-[10px] font-bold text-violet-300 hover:bg-violet-500/10 light:text-violet-700"
            >
              Advance status
            </button>
          </div>
        ...
        {filtered.length === 0 && (
          <EmptyMessage text="No sample activity matches your filters." />
        )}
      </div>
    </section>
  );
...
function CatalogSection({
  ...
  return (
    <section className={cn(panelClass, "overflow-hidden")}>
      <div className="flex flex-wrap items-center gap-3 border-b border-slate-700 p-4 light:border-slate-200">
        <SearchBox
          value={search}
          onChange={onSearch}
          placeholder={`Search ${isAppointment ? "services" : "products"}...`}
        />
        <button
          onClick={onAdd}
          className="ml-auto inline-flex items-center gap-2 rounded-xl bg-violet-600 px-4 py-2.5 text-xs font-bold text-white hover:bg-violet-500"
        >
          <Plus className="h-4 w-4" />
          Add demo {isAppointment ? "service" : "product"}
        </button>
      </div>
      <div className="grid gap-4 p-4 sm:grid-cols-2 xl:grid-cols-3">
        {filtered.map((item) => (
          <article
            key={item.id}
            className="overflow-hidden rounded-xl border border-slate-700 bg-slate-800/50 light:border-slate-200 light:bg-slate-50"
          >
            ...
            <div className="p-4">
              ...
              <button
                ...
                className={cn(
                  "mt-4 w-full rounded-lg px-3 py-2 text-xs font-bold",
                  item.active
              ...
              >
                {item.active
                  ? "Available — click to turn off"
                  : "Unavailable — click to turn on"}
              </button>
            </div>
          </article>
        ))}
        {filtered.length === 0 && (
          <div className="sm:col-span-2 xl:col-span-3">
            <EmptyMessage text="No sample catalog items match your search." />
          </div>
        )}
      </div>
    </section>
...
function CustomersSection({
  ...
  const filtered = customers.filter((customer) =>
    [customer.name, customer.contact].some((value) =>
      value.toLowerCase().includes(search.toLowerCase()),
    ),
  );
  return (
    <section className={cn(panelClass, "overflow-hidden")}>
      <div className="border-b border-slate-700 p-4 light:border-slate-200">
        <SearchBox
          value={search}
          onChange={onSearch}
      ...
      <div className="overflow-x-auto">
              ...
              <tr key={customer.id}>
                <td className="px-5 py-4">
                  <div className="flex items-center gap-3">
                    <span className="flex h-8 w-8 items-center justify-center rounded-full bg-violet-500/15 text-[10px] font-bold text-violet-300">
                      {initials(customer.name)}
                    </span>
                    <span className="font-semibold">{customer.name}</span>
                  </div>
                </td>
                <td className="px-5 py-4 text-slate-500">{customer.contact}</td>
                <td className="px-5 py-4">{customer.visits}</td>
        ...
        {filtered.length === 0 && (
          <EmptyMessage text="No sample customers match your search." />
        )}
      </div>
    </section>
  );
    ...
    <div className="space-y-4">
      <section className="grid gap-3 sm:grid-cols-3">
        ...
        <DemoStat
          ...
          value={`+${analytics.activityChange}%`}
          color="blue"
        />
      </section>
      <section className={cn(panelClass, "p-5")}>
        ...
        <p className="mt-1 text-[10px] text-slate-500">
          Hover over the bars to inspect the sample values
        </p>
        <div className="mt-5 flex h-60 items-end gap-3 border-b border-slate-700 px-3 light:border-slate-200">
          {analytics.revenueData.map((point) => (
            ...
            >
              <div
                ...
                title={`${point.date}: ${formatCurrency(point.revenue)} · ${point.count} activities`}
              />
            </div>
          ))}
        </div>
      ...
      <section className={cn(panelClass, "p-5")}>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {analytics.topItems.map((item, index) => (
            <div
              key={item.name}
              className="flex items-center gap-3 rounded-xl bg-slate-800/50 p-3 light:bg-slate-50"
            >
              <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-violet-500/15 text-xs font-bold text-violet-300">
                #{index + 1}
              </span>
              <div className="min-w-0 flex-1">
                <p className="truncate text-xs font-semibold">{item.name}</p>
                <p className="text-[10px] text-slate-500">{item.count} sales</p>
              </div>
              <span className="text-xs font-bold">
  ...
  return (
    <section className={cn(panelClass, "p-5 sm:p-6")}>
      <div className="max-w-2xl">
        <h3 className="text-sm font-bold">Storefront settings preview</h3>
        <p className="mt-1 text-xs text-slate-500">
          Try editing these fields. Changes stay only in this browser session.
        </p>
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <DemoField
            label="Business name"
            value={form.businessName}
            onChange={(value) => onChange({ ...form, businessName: value })}
          />
          <DemoField
            label="Phone number"
            value={form.phone}
          ...
          <label className="sm:col-span-2">
            ...
            <textarea
              ...
              rows={4}
              className="w-full rounded-xl border border-slate-700 bg-slate-800 p-3 text-xs outline-none focus:border-violet-500 light:border-slate-200 light:bg-slate-50"
            />
          </label>
          <label>
            <span className="mb-1.5 block text-xs font-semibold">
            ...
            <div className="flex items-center gap-3 rounded-xl border border-slate-700 bg-slate-800 p-2 light:border-slate-200 light:bg-slate-50">
              <input
                type="color"
                value={form.primaryColor}
                onChange={(event) =>
                  onChange({ ...form, primaryColor: event.target.value })
                }
        ...
        <button
          onClick={onSave}
          className="mt-5 inline-flex items-center gap-2 rounded-xl bg-violet-600 px-5 py-3 text-xs font-bold text-white hover:bg-violet-500"
        >
...
function ActivityRow({
  ...
  return (
    <div
      className={cn("flex min-w-0 items-center gap-3", !compact && "px-5 py-3")}
    >
...
function DemoStat({
  ...
  return (
    <div className={cn(panelClass, "p-4")}>
      <div className={cn("inline-flex rounded-xl p-2.5", colors)}>
        <Icon className="h-4 w-4" />
      </div>
  ...
  return (
    <label className="relative min-w-0 flex-1 sm:min-w-[220px]">
      ...
      <input
        ...
        className="w-full rounded-xl border border-slate-700 bg-slate-800 py-2.5 pl-10 pr-3 text-xs outline-none focus:border-violet-500 light:border-slate-200 light:bg-slate-50"
      />
    </label>
  );
}
  ...
  return (
    <label>
      ...
      <input
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2.5 text-xs outline-none focus:border-violet-500 light:border-slate-200 light:bg-slate-50"
      />
    </label>
  );
}
...
function EmptyMessage({ text }: { text: string }) {
  return (
    <p className="px-5 py-12 text-center text-xs text-slate-500">{text}</p>
  );
}
