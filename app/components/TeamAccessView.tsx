...
const inputClass =
  "h-11 w-full rounded-xl border border-slate-600 bg-slate-900/70 px-3 text-sm text-white outline-none transition placeholder:text-slate-500 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/15 light:border-slate-300 light:bg-white light:text-slate-900";
...
export function TeamAccessView({ tenant }: { tenant: Tenant }) {
  ...
  const invite = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setInvitationLink("");
    try {
      const invitation = await createBusinessTeamInvitation(
        tenant.id,
        email,
        role,
      );
  ...
  const copyInvitation = async () => {
    try {
      ...
      setError(
        "Copy was blocked. Select and copy the invitation link manually.",
      );
    }
  };
  ...
  const removeMember = async (membershipId: string, name: string) => {
    if (
      !window.confirm(
        `Remove ${name}'s access to ${tenant.name}? Their historical activity will remain stored.`,
      )
    )
      return;
    setBusyId(membershipId);
  ...
  if (isLoading && !summary) {
    return (
      <Card>
        <CardBody className="flex min-h-48 items-center justify-center gap-2 text-sm text-slate-400">
          <Loader2 className="h-4 w-4 animate-spin" /> Loading Team & Access...
        </CardBody>
      </Card>
    );
  }
    ...
    return (
      <Card>
        <CardBody>
          <p
            role="alert"
            className="rounded-xl bg-rose-500/10 p-4 text-sm text-rose-300 light:text-rose-700"
          >
            {error || "Unable to load Team & Access."}
          </p>
        </CardBody>
      </Card>
    );
    ...
    <div className="space-y-4">
      <Card>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h3 className="flex items-center gap-2 text-sm font-bold text-white light:text-slate-900">
                <Users className="h-4 w-4 text-violet-400" /> Team & Access
              </h3>
              <p className="mt-1 text-[11px] text-slate-400 light:text-slate-600">
                You control who can access this business. YuhBusiness controls
                subscription and seat authorization.
              </p>
            </div>
            <span className="rounded-full bg-violet-500/15 px-3 py-1 text-[10px] font-bold text-violet-300 light:text-violet-700">
              {planName} plan
            </span>
          </div>
        </CardHeader>
        <CardBody className="space-y-5">
          <div className="grid gap-3 sm:grid-cols-3">
            <SeatStat
              label="Active staff"
              value={`${summary.activeStaff} / ${summary.authorizedStaff}`}
            />
            <SeatStat
              label="Included staff"
              value={String(summary.includedStaff)}
            />
            <SeatStat
              label="Paid seat charge"
              value={`$${summary.monthlySeatCharge} BZD/mo`}
            />
          </div>
          ...
          {summary.maxStaff === 0 ? (
            <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm leading-6 text-amber-100 light:text-amber-800">
              Beginner is owner-only. Upgrade this business to Pro for one
              included staff member or Enterprise for two.
            </div>
          ) : (
            <form
              onSubmit={invite}
              className="rounded-2xl border border-slate-700 bg-slate-900/35 p-4 light:border-slate-200 light:bg-slate-50"
            >
              <div className="flex items-center gap-2">
                <UserPlus className="h-4 w-4 text-violet-400" />
                <h4 className="text-xs font-bold">Invite a team member</h4>
              </div>
              <p className="mt-1 text-[11px] leading-5 text-slate-400 light:text-slate-600">
                Invitations are locked to the entered email, expire after 72
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_9rem_auto]">
                ...
                <select
                  value={role}
                  onChange={(event) =>
                    setRole(event.target.value as BusinessTeamRole)
                  }
                ...
                <button
                  disabled={busyId === "invite" || availableSeats === 0}
              ...
              <p className="mt-2 text-[10px] text-slate-500">
                {availableSeats} invitation seat
                {availableSeats === 1 ? "" : "s"} currently available. Pending
                invitations reserve a seat but are not billed.
              </p>
            </form>
          )}
          ...
          {invitationLink && (
            <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4">
              <p className="flex items-center gap-2 text-xs font-bold text-emerald-300 light:text-emerald-800">
                <CheckCircle2 className="h-4 w-4" /> Secure invitation ready
              </p>
              <p className="mt-1 text-[11px] text-emerald-100/80 light:text-emerald-800">
                The invitation is emailed when delivery is configured. You can
                also send this secure link directly to the invited email
                address.
              </p>
              <div className="mt-3 flex gap-2">
                ...
                <button
                  ...
                  className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 text-xs font-bold text-white hover:bg-emerald-500"
                >
          ...
          {error && (
            <p
              role="alert"
              className="rounded-xl bg-rose-500/10 px-3 py-2 text-xs text-rose-300 light:text-rose-700"
            >
              {error}
            </p>
          )}
          {success && (
            <p
              role="status"
              className="rounded-xl bg-emerald-500/10 px-3 py-2 text-xs text-emerald-300 light:text-emerald-700"
            >
              {success}
            </p>
          )}
      ...
      <Card>
        <CardHeader>
          <h3 className="flex items-center gap-2 text-xs font-bold">
            <ShieldCheck className="h-4 w-4 text-violet-400" /> Business members
          </h3>
        </CardHeader>
        <CardBody className="space-y-3">
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="truncate text-[11px] text-slate-400 light:text-slate-600">
                    {member.email}
                  </p>
                </div>
                {isOwner ? (
                  <span className="rounded-full bg-violet-500/15 px-3 py-1 text-[10px] font-bold uppercase text-violet-300 light:text-violet-700">
                    Owner
                  </span>
                ) : (
                  <div className="flex items-center gap-2">
                    <select
                      ...
                      onChange={(event) =>
                        void changeRole(
                          member.membershipId,
                          event.target.value as BusinessTeamRole,
                        )
                      }
                      aria-label={`Role for ${member.name}`}
                      className="h-9 rounded-lg border border-slate-600 bg-slate-900 px-2 text-xs text-white light:border-slate-300 light:bg-white light:text-slate-900"
                    >
                      <option value="staff">Staff</option>
                      <option value="manager">Manager</option>
                    </select>
                    <button
                      ...
                      disabled={busy}
                      onClick={() =>
                        void removeMember(member.membershipId, member.name)
                      }
                      aria-label={`Remove ${member.name}`}
                      title={`Remove ${member.name}`}
                      className="flex h-9 w-9 items-center justify-center rounded-lg border border-rose-500/30 text-rose-300 hover:bg-rose-500/10 disabled:opacity-50 light:text-rose-700"
                    >
                      {busy ? (
                        <Loader2 className="h-4 w-4 animate-spin" />
                      ) : (
          ...
          {formerMembers.length > 0 && (
            <p className="pt-2 text-[10px] font-bold uppercase tracking-wider text-slate-500">
              Former members retained for history: {formerMembers.length}
            </p>
          )}
        </CardBody>
      </Card>
      ...
      {summary.invitations.length > 0 && (
        <Card>
          <CardHeader>
            <h3 className="flex items-center gap-2 text-xs font-bold">
              <Clock3 className="h-4 w-4 text-amber-400" /> Pending invitations
            </h3>
          </CardHeader>
          <CardBody className="space-y-2">
            {summary.invitations.map((invitation) => (
              <div
                key={invitation.id}
                className="flex flex-wrap items-center gap-3 rounded-xl border border-slate-700 p-3 light:border-slate-200"
              >
                <div className="min-w-0 flex-1">
                ...
                <button
                  ...
                  className="rounded-lg border border-slate-600 px-3 py-2 text-[10px] font-bold text-slate-300 hover:border-rose-500/40 hover:text-rose-300 disabled:opacity-50 light:border-slate-300 light:text-slate-700"
                >
                  Revoke
                </button>
              </div>
      ...
      {maxPaidSeats > 0 && (
        <Card>
          <CardHeader>
            <h3 className="flex items-center gap-2 text-xs font-bold">
              <CreditCard className="h-4 w-4 text-emerald-400" /> Additional
              staff seats
            </h3>
          </CardHeader>
          <CardBody>
            ...
            {summary.pendingSeatRequest ? (
              <div className="mt-4 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-xs text-amber-100 light:text-amber-800">
                ...
                {summary.pendingSeatRequest.requestedPaidSeats === 1
                  ...
                  : "s"}{" "}
                ($
                {summary.pendingSeatRequest.requestedPaidSeats *
...
function SeatStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-slate-700 bg-slate-900/35 p-4 light:border-slate-200 light:bg-slate-50">
