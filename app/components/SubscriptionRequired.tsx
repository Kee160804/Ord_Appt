    ...
    <div className="pwa-page-roomy min-h-dvh bg-[#070b14] px-4 py-8 text-white light:bg-[#f6f8fc] light:text-slate-900 sm:px-6">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {plans.map((plan) => (
            ...
            >
              ...
              {plan.checkoutUrl ? (
                <a
                  href={checkoutHref(plan.checkoutUrl, tenant, plan.id)}
                  className={`mt-auto inline-flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-xs font-bold text-white ${plan.popular ? "bg-violet-600 hover:bg-violet-500" : "bg-slate-700 hover:bg-slate-600"}`}
                >
        ...
        <div className="mx-auto mt-8 flex max-w-2xl items-start gap-3 rounded-2xl border border-slate-700 bg-slate-900/50 p-4 text-xs leading-5 text-slate-400 light:border-slate-200 light:bg-white light:text-slate-600">
          ...
          <p>
            {hasCheckout
              ? "Secure checkout opens through the configured payment provider. Access is restored after the payment is confirmed."
              : "Online checkout links have not been configured yet. Review the plans, then contact the platform administrator to arrange payment and restore access."}
          </p>
        </div>
      </div>
    </div>
