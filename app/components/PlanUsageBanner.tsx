...
import {
  ...
  type MonthlyUsage,
} from "@/app/services/usageService";
...
export function PlanUsageBanner({ tenant }: { tenant: Tenant }) {
  ...
  const warningLevel = usage.isLimitReached
    ? 100
  ...
  return (
    <div className={`border-b px-4 py-2 text-[11px] ${containerClass}`}>
      <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-center gap-x-3 gap-y-2">
        ...
        <span>
          <strong>
            {usage.activityCount} of {usage.activityLimit}
          </strong>{" "}
          monthly orders or appointments used on the {planName} plan.
        </span>
        <div
          className="h-1.5 w-28 overflow-hidden rounded-full bg-black/15 light:bg-black/10"
          aria-hidden="true"
        >
          <div
        ...
        {warningLevel === 80 && (
          <span className="font-semibold">
            You have used at least 80% of this month&apos;s allowance.
          </span>
        )}
        {warningLevel === 90 && (
          <span className="font-semibold">
            You are close to this month&apos;s limit.
          </span>
        )}
        {warningLevel === 100 && (
          <span className="font-semibold">
            New public activity is paused until next month or a plan change.
          </span>
        )}
        {warningLevel >= 80 && (
          <Link
