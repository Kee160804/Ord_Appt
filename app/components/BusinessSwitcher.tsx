...
export function BusinessSwitcher({
  ...
  const { user, businesses, switchBusiness, addBusiness, isSwitchingBusiness } =
    useAuth();
  ...
  return (
    <>
      <button
        ...
        title={
          collapsed ? `Switch business: ${tenant.name}` : "Switch business"
        }
        ...
        {adding ? (
          <form onSubmit={submitBusiness} className="space-y-5">
            <div className="rounded-2xl border border-violet-500/25 bg-violet-500/10 p-4 text-xs leading-5 text-violet-100 light:text-violet-900">
              This creates a separate business under the same login. It starts
              on a 14-day trial and keeps its own data, plan, and subscription.
            </div>
            ...
            <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              ...
              <button
                disabled={isSwitchingBusiness || businessName.trim().length < 2}
          ...
          <div>
            <p className="mb-3 text-xs leading-5 text-slate-400 light:text-slate-600">
              Your businesses use one account, but their customers, activity,
              settings, storefronts, and subscriptions stay separate.
            </p>
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="mt-0.5 block text-[10px] text-slate-400 light:text-slate-500">
                        {plan.name} · ${plan.price} BZD/month ·{" "}
                        {business.businessType === "appointment"
                          ? "Appointments"
                          : "Ordering"}
                      </span>
                    </span>
                    {isSwitchingBusiness && !active ? (
            ...
            {error && (
              <p
                role="alert"
                className="mt-3 rounded-xl bg-rose-500/10 px-3 py-2 text-xs text-rose-300 light:text-rose-700"
              >
                {error}
              </p>
            )}
