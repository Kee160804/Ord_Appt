      ...
      <main className="w-full max-w-md rounded-3xl border border-slate-800 bg-slate-900/70 p-7 shadow-2xl light:border-slate-200 light:bg-white sm:p-9">
        ...
        {sent ? (
          ...
          <>
            ...
            <button disabled={isSending} onClick={() => void sendReset()} className="mt-5 w-full rounded-xl bg-violet-600 px-4 py-3 text-sm font-bold text-white hover:bg-violet-500 disabled:opacity-50">
              {isSending ? "Sending reset link..." : "Send reset link"}
            </button>
            <Link href="/login" className="mt-5 flex items-center justify-center gap-2 text-sm font-semibold text-violet-400 hover:text-violet-300 light:text-violet-700">
              <ArrowLeft className="h-4 w-4" /> Back to sign in
            </Link>
          </>
        )}
