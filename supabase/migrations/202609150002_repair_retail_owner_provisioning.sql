...
-- Existing projects may have installed provision_owner_business before retail
-- was introduced. Editing an already-applied migration does not update the
-- live function, so publish the current idempotent definition forward-only.
...
BEGIN
  ...
  IF v_slug = '' THEN
    v_slug := 'business-' || LEFT(REPLACE(v_user_id::TEXT, '-', ''), 12);
  END IF;

  -- Login hydration and auth callbacks may provision concurrently. Serialize
  -- them and return an existing active membership before creating anything.
  ...
  -- The tenant initializer may already have created OWNER and its membership.
  -- Reuse that data when present and safely fill either missing record.
...
REVOKE ALL ON FUNCTION public.provision_owner_business(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT)
FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.provision_owner_business(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT)
TO authenticated;
