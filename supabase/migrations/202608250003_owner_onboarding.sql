...
BEGIN
  ...
  -- Foundational tenant triggers may already create this OWNER membership.
  -- Update first so trigger-created rows are reused without attempting a
  -- duplicate insert, then retain an exact-constraint upsert for trigger-free
  -- installations and simultaneous retries.
