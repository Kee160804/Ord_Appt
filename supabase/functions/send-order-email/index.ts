...
const money = (value: number | null | undefined) =>
  new Intl.NumberFormat("en-US", { style: "currency", currency: "BZD" }).format(Number(value ?? 0));

const EVENT_COPY: Record<string, { heading: string; message: string }> = {
  ORDER_RECEIVED: { heading: "Order received", message: "We received your order and will review it shortly." },
  ORDER_ACCEPTED: { heading: "Order accepted", message: "Your order has been accepted." },
  ORDER_PREPARING: { heading: "Being prepared", message: "Your order is now being prepared." },
  ORDER_READY: { heading: "Order ready", message: "Your order is ready." },
  ORDER_OUT_FOR_DELIVERY: { heading: "Out for delivery", message: "Your order is on its way." },
  ORDER_COMPLETED: { heading: "Order complete", message: "Your order has been completed. Thank you!" },
  ORDER_CANCELLED: { heading: "Order cancelled", message: "This order was cancelled." },
};
...
function buildEmail(delivery: DeliveryRow) {
  ...
  const fulfillment = [
    ...
    payload.requested_time ? new Date(payload.requested_time).toLocaleString("en-US") : "",
  ].filter(Boolean).join(" · ");
  const cancellation = delivery.event_type === "ORDER_CANCELLED" && payload.cancellation_reason
  ...
  const text = [
    `Hi ${delivery.recipient_name},`, copy.message,
    `Order: ${payload.order_number}`,
    fulfillment ? `Fulfillment: ${fulfillment}` : "",
    ...(payload.items ?? []).map((item) => `${item.quantity} × ${item.name}: ${money(item.subtotal)}`),
...
export default {
  async fetch(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (!safeEqual(providedSecret, requiredEnv("NOTIFICATION_WEBHOOK_SECRET"))) {
        return json({ error: "Unauthorized." }, 401);
      }
      const body = (await request.json()) as WebhookBody;
      if (body.type !== "INSERT" || body.schema !== "public" || body.table !== "order_email_deliveries" || !body.record?.id) {
      ...
      const columns = "id, order_id, event_type, recipient_email, recipient_name, subject, payload, status, attempt_count";
      const { data: currentData, error: readError } = await supabase.from("order_email_deliveries").select(columns).eq("id", body.record.id).single();
      ...
      const { data: claimedData, error: claimError } = await supabase.from("order_email_deliveries").update({
        status: "PROCESSING",
        attempt_count: current.attempt_count + 1,
        processing_started_at: new Date().toISOString(),
        last_error: null,
      }).eq("id", current.id).in("status", ["PENDING", "FAILED"]).select(columns).maybeSingle();
      ...
      if (!resendResponse.ok || !result.id) {
        const providerError = result.message || `Resend returned ${resendResponse.status}.`;
        await supabase.from("order_email_deliveries").update({ status: "FAILED", last_error: providerError.slice(0, 1000) }).eq("id", delivery.id);
        return json({ error: providerError }, 502);
      }
      const { error: sentError } = await supabase.from("order_email_deliveries").update({ status: "SENT", provider_message_id: result.id, sent_at: new Date().toISOString(), last_error: null }).eq("id", delivery.id);
